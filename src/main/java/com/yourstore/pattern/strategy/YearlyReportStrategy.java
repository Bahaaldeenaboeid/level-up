...
public class YearlyReportStrategy implements ReportStrategy {
    ...
    private static final Logger logger = LoggerFactory.getLogger(YearlyReportStrategy.class);

    @Override
    public AdminReportResponse generate() {
        logger.info("Generating yearly admin report");
        LocalDate now = LocalDate.now();
        LocalDate startOfYear = now.withDayOfYear(1);
        AdminReportResponse response = new AdminReportResponse();
        response.setReportType("YEARLY");
        response.setStartDate(startOfYear);
